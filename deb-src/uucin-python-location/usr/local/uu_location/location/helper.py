...
WIFI_DB = leveldb.LevelDB(os.path.join(here, '../data/wifi')).CreateSnapshot()
CELL_DB = leveldb.LevelDB(os.path.join(here, '../data/cell')).CreateSnapshot()
GEO_TOOLS = GeoTools()
    ...
    for mac, rssi in keys:
        try:
            ...
            if -110 < rssi < -20:
                #A = 45 ~ 49
                n = 3.25  # 3.25~4.5
                A = 110 - math.log(accuracy, 10) * (10 * n)
                accuracy = 10 ** ((abs(rssi) - A) / (10 * n))
    ...
    if wifi_chunks:
        for cell_chunk in cell_chunks:
            for wifi_chunk in wifi_chunks:
                cell_point = cell_chunk.avg()
                wifi_point = wifi_chunk.avg()
